...
| 컬럼명 | 열 | 타입 | 필수 | 제약 조건 | 설명 |
|:---|:---:|:---|:---:|:---|:---|
| `Date` | A | string | Y | `YYYY-MM-DD` 형식 | 거래 날짜 |
| `Ticker` | B | string | Y | 비어 있으면 안 됨 | 한국어 종목명 |
| `Type` | C | string | Y | `매수` 또는 `매도` (BR-013) | 거래 유형 |
| `Quantity` | D | number | Y | 양의 정수 | 거래 수량 |
| `Price` | E | number | Y | 양의 실수 | 거래 단가 (원) |
| `Fee` | F | number | N | 0 이상 | 수수료 (BR-018) |
| `Tax` | G | number | N | 0 이상, 매도 시 발생 | 거래세 (BR-017) |
| `Journal` | H | string | N | — | 매매 메모 |
| `Tags` | I | string | N | 콤마(`,`) 구분 | 전략 태그 목록 |
| `Amount` | J | number | Y | Quantity × Price | 거래대금 — 수수료·세금 미포함 (BR-019) |
...
export interface SheetTransactionRow {
  ...
  Price: number;
  Fee: number;          // 수수료 (F열)
  Tax: number;          // 거래세 (G열, 매도 시 발생)
  Journal: string;
  Tags: string;         // 콤마 구분 문자열
  Amount: number;       // 거래대금 = Quantity × Price (J열)
}
```

### 2.3 샘플 데이터

| Date | Ticker | Type | Quantity | Price | Fee | Tax | Journal | Tags | Amount |
|:---|:---|:---|:---|:---|:---|:---|:---|:---|:---|
| 2024-01-15 | 삼성전자 | 매수 | 10 | 72000 | 540 | 0 | 반도체 회복 기대 | 성장주,반도체 | 720000 |
| 2024-03-20 | 삼성전자 | 매도 | 5 | 78000 | 390 | 234 | 목표가 달성 | 성장주,반도체 | 390000 |
